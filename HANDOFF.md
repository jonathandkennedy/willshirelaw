...
**Blockers**
- [ ] **Form backend.** `content/site.json → form`. On Netlify nothing is needed (Netlify Forms; add email/SMS notifications in the dashboard, and route to an inbox intake watches 24/7). On Vercel or any other host set `formspree_id` (or a CRM/Zapier `endpoint`) and rebuild — otherwise submissions go nowhere. Test with a real phone: the lead must arrive with subject `WilshirePPC — <practice> — <city> — <name>`.
